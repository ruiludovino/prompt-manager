...
import { createPrompt, updatePrompt, createCategory } from "@/lib/actions";
...
export function PromptEditorForm({
  ...
  const handleSave = async () => {
    ...
    try {
      if (isEditing && existing) {
        await updatePrompt(existing.id, payload);
        router.push(`/prompts/${existing.id}`);
      } else {
        const id = await createPrompt(payload);
  ...
  const handleCancel = () => {
    if (isEditing && existing) router.push(`/prompts/${existing.id}`);
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          ...
          <button
            onClick={handleSave}
            disabled={saving}
            className="rounded-lg bg-primary px-3.5 py-2 text-sm font-medium text-white transition hover:bg-primary-hover disabled:opacity-60"
          >
      ...
      <div className="mt-6 grid grid-cols-1 gap-6 lg:grid-cols-3">
        <div className="space-y-5 lg:col-span-2">
          ...
          {detectedVariables.length > 0 && (
            <div className="rounded-xl border border-border bg-surface p-4">
              <h2 className="font-display text-sm font-semibold text-text">Detected Variables</h2>
                ...
                {detectedVariables.map((v) => (
                  <div key={v} className="flex items-center gap-3">
                    ...
                    <input
                      ...
                      onChange={(e) => setVariableNotes((prev) => ({ ...prev, [v]: e.target.value }))}
        ...
        <div className="space-y-5">
          ...
          <div className="rounded-xl border border-border bg-surface p-4">
            ...
            {visibility === "team" && (
              <p className="mt-2 text-xs text-text-faint">
                Everyone on your team will get an email when you share this.
              </p>
            )}
          </div>
          ...
          <div className="flex items-start gap-2 rounded-xl border border-border-soft bg-secondary-soft p-4 text-xs text-text-muted">
            ...
            <span>
              Tip: Use <code className="text-secondary">{"{{double curly braces}}"}</code> to mark
              variables users will fill in. See{" "}
